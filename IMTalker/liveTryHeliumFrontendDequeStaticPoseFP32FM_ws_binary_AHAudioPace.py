...
class MoshiOnlyEngineWithHidden(MoshiOnlyEngine):
    ...
    def reset_session(self) -> None:
        super().reset_session()
        # Per-conversation RAG/STT turn-detection state. No-ops harmlessly
        # when RAG isn't configured (self.stt_lm_gen stays None).
    ...
    def _rag_stt_step(self, chunk: torch.Tensor) -> None:
        ...
        kick off a RAG turn. Never touches self.lm_gen/self.mimi."""
        ...
        if vad_score > self.vad_threshold:
            ...
            self.stt_silence_frame_count = 0
        vad_fired = (
            self.stt_silence_frame_count >= self._RAG_VAD_SILENCE_FRAMES_REQUIRED
            and not self.stt_last_vad_end
        )
        self.stt_last_vad_end = self.stt_silence_frame_count >= self._RAG_VAD_SILENCE_FRAMES_REQUIRED
        if vad_fired and self.stt_in_utterance and self.stt_token_buffer and not self.rag_awaiting_ref:
            import rag_helpers

            transcript = rag_helpers.decode_stt_tokens(
            ...
            if transcript.strip():
                ...
                self._rag_start_turn(transcript)

    def _rag_start_turn(self, transcript: str) -> None:
        """Cheap FAISS gate (local embed + search only, a few ms) decides
        whether this turn is worth a <ref> injection at all. If it clears the
        gate: inject a <lookup> filler burst synchronously (right here, on the
        GPU thread), then hand the transcript to a background thread for the
        ...
        top_score = float(max(valid)) if valid else 0.0
        ...
        if not triggered:
            ...
            # _rag_stt_step captures casual (non-RAG) turns too.
        ...
        self.rag_turn_epoch += 1
        my_epoch = self.rag_turn_epoch
        self.rag_ref_committed_this_turn = False
        self.rag_awaiting_ref = True
        self.rag_filler_frame_count = 0
        self.rag_current_transcript = transcript
        ...
        print(
            ...
            f"transcript={transcript!r} -- retrieving in background",
            flush=True,
        )

        threading.Thread(
            target=self._rag_retrieve_and_compress,
            args=(transcript, my_epoch),
            daemon=True,
            name="rag-retrieval",
        ).start()

    def _rag_retrieve_and_compress(self, transcript: str, my_epoch: int) -> None:
        """Background thread: local FAISS retrieval, optional web-search
        fallback, then compression into a short grounding statement. Only
        touches FAISS/embedding/compressor objects and plain attributes --
        never self.lm_gen/self.mimi/self.stt_lm_gen. Writes the result into
        self.rag_pending_ref_tokens, which the GPU thread polls each chunk."""
        import rag_helpers
    ...
    def _rag_consume_pending_ref(self) -> None:
        ...
        if self.rag_pending_ref_tokens is not None:
            ref_tokens = self.rag_pending_ref_tokens
            self.rag_pending_ref_tokens = None
            self.rag_awaiting_ref = False
        ...
        elif self.rag_filler_frame_count >= self._RAG_MAX_FILLER_FRAMES:
...
class LiveHeliumFMOptions(BaseOptions):
    def initialize(self, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        # RAG / tool-calling / web-search (all optional; omitting every flag
        # below reproduces the exact pre-RAG behavior of this script)
        parser.add_argument("--rag_checkpoint_dir", default="", help="Dir containing lora/ with the RAG LoRA adapter")
        parser.add_argument("--merge_rag_lora", action="store_true", help="Merge RAG LoRA into base weights instead of keeping it unmerged (QLoRA-style; default is unmerged)")
        parser.add_argument("--rag_index_dir", default="", help="Path to a RAG index (manifest.json + chunks.npz). Omit to disable RAG.")
        ...
        parser.add_argument("--compressor_model", default="", help="HF model id for the RAG context compressor, e.g. Qwen/Qwen2.5-1.5B-Instruct. Omit to use extractive summarization only.")
    ...
    def get_moshi_engine() -> MoshiOnlyEngine:
        ...
        if moshi_engine is None:
            moshi_engine = MoshiOnlyEngineWithHidden(
                ...
                rag_lora_dir=getattr(args, "rag_checkpoint_dir", ""),
                merge_rag_lora=bool(getattr(args, "merge_rag_lora", False)),
                rag_index_dir=getattr(args, "rag_index_dir", ""),
                rag_top_k=int(getattr(args, "rag_top_k", 2)),
                rag_min_score=float(getattr(args, "rag_min_score", 0.25)),
                rag_trigger_score=float(getattr(args, "rag_trigger_score", 0.30)),
                    ...
                    for ev in reply_engine.process_ready_steps_limited(max_moshi_steps_per_loop):
                        ...
                        if reply_avatar_chunk_idx % 50 == 0:
                            status_kwargs = {
                                ...
                                "rag_enabled": reply_engine.rag_enabled,
