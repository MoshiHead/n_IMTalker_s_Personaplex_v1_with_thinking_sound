...
# --- RAG additions -----------------------------------------------------------
# Optional: only needed when the live server is launched with ENABLE_RAG=1.
# Never fails the whole asset pass if the RAG dataset repo is unreachable —
# RAG is additive, the avatar must still be able to boot without it.
RAG_LORA_DIR="${RAG_LORA_DIR:-$CHECKPOINT_DIR/rag_lora}"
echo "[11/11] Downloading RAG LoRA adapter (Darknsu/helium_lora_v1)..."
if mkdir -p "$RAG_LORA_DIR/lora" && hf download Darknsu/helium_lora_v1 \
  adapter_model.safetensors \
  --repo-type dataset \
  --local-dir "$RAG_LORA_DIR/lora"
then
  ...
  cat > "$RAG_LORA_DIR/lora/adapter_config.json" <<'JSON'
{
...
JSON
  echo "  RAG LoRA ready: $RAG_LORA_DIR/lora"
else
  echo "  [warn] RAG LoRA download failed/unreachable -- continuing without RAG." >&2
  echo "         Re-run this script later, or launch with ENABLE_RAG=0." >&2
fi
...
echo "  RAG LoRA (optional): $RAG_LORA_DIR/lora"
echo "  RAG index (git-tracked, no download needed): $ROOT/personaplex/rag_index"
